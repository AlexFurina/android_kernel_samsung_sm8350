...
static int __init hdm_flag_setup(char *hdm_status)
{
	...
	if (hdm_status) {
		...
		while (token) {
			...
			if(cnt++%2) {
				if (simple_strtol(token,NULL,16) & HDM_WIFI_SUPPORT_BIT && hdm_wifi_support == 0) {
					hdm_info("%s wifi bit set applied bit = 0x%x\n", __func__, simple_strtol(token,NULL,16));
					hdm_wifi_support = 1;
				}
				if (simple_strtol(token,NULL,16) & HDM_CP_SUPPORT_BIT && hdm_cp_support == 0) {
					hdm_info("%s cp bit set applied bit = 0x%x\n", __func__, simple_strtol(token,NULL,16));
					hdm_cp_support = 1;
				}
			}	
			token = strsep(&tmp_p, "&|");
	...
	struct device_attribute *attr, char *buf)
{
	...
	bt_block_sub = block_type_bit & 0xFFF;
	...
	struct device_attribute *attr, char *buf)
{
	...
	bt_unblock_sub = (block_type_bit >> 12) & 0xFFF;
